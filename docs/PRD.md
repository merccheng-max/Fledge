...
- **Chrome extension — trip fit score.** While shopping on a retailer site, the user clicks "Check fit for [trip]" on a product and gets a 0–100 score for how well that specific item fits the requirements Fledge already computed for their trip (park, season, group size) — not a review-sentiment score. It cross-references the product's extracted specs (capacity, temperature rating, weight, etc.) against the checklist engine's quantity/spec reasoning, and against what's already covered vs. still missing on the user's checklist, so it can also say "you already have this covered" or "this would fill your still-missing water filter slot." Deferred deliberately: a browser extension is a separate technical surface from the current web stack, spec-extraction is realistically scoped to one retailer's DOM at a time (REI first, matching the existing shop links), and scoring against "which trip" requires some way to reference a specific trip from outside the web app — most likely a shareable trip ID, which nudges toward light trip persistence even without full accounts. Revisit after the core web app and case-study materials are complete.
